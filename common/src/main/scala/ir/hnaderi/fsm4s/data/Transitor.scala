...
final class TransitionDSL[S, Ev, TEv] {
  ...
  def emit(ev: TEv, evs: TEv*): F[Unit] =
    RWS.tell(Chain(ev) ++ Chain.fromSeq(evs))
  def ask: F[Ev] = RWS.ask
  def next: F[(S, Ev)] = read product ask
  def pure[T](t: T): F[T] = RWS.pure(t)
  def goTo(s: S, evs: TEv*): F[Unit] =
    RWS.set[Ev, Chain[TEv], S](s).tell(Chain.fromSeq(evs))
